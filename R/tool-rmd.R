    ...
    miniUI::miniTabstripPanel(
      ...
      miniUI::miniTabPanel(
        ...
        miniUI::miniContentPanel(
          ...
          shiny::sliderInput(inputId = "est_n_boot", value = 1000, min = 100, max = 1000, step = 100,
                             label = help_label("Number of Bootstrap Samples", "Used in calculating uncertainty but affects run time. 1000 should be used for final versions but fewer may be used in trial versions.")),
          shiny::checkboxInput(inputId = "est_draft", value = TRUE,
                               label = help_label("Mark as Draft", "If checked, a watermark will be included on the output document.")),
          shiny::checkboxInput(inputId = "est_do_setnets", value = TRUE,
                               label = help_label("Make Set Net Harvest Estimate", "Under rare circumstances, you may deem the number of set net interviews insufficient to produce an estimate. In that case, uncheck this box.")),
          shiny::checkboxInput(inputId = "est_include_johnson_table", value = ifelse(meta$set_only, FALSE, TRUE),
                               label = help_label("Include Johnson River Proximity Table", "Uncheck if you do not wish to include a table showing estimated catch per trip and species composition upstream and downstream of the Johnson River confluence. Valid for drift nets only.")),
          shiny::checkboxInput(inputId = "est_include_goal_table", value = TRUE,
                               label = help_label("Include Harvest Goal Attainment Table", "Uncheck if you do not wish to include a table showing the percent of fishers reporting their attainment of harvest goals.")),
          shiny::checkboxInput(inputId = "est_include_appendix", value = TRUE,
                               label = help_label("Include Detailed Appendix", "Uncheck if you do not wish to include the appendix showing detailed area-specific summaries of the various interview responses.")),
      ...
      miniUI::miniTabPanel(
        ...
        miniUI::miniContentPanel(
          ...
          shiny::sliderInput(inputId = "sen_n_boot", value = 1000, min = 100, max = 1000, step = 100,
                             label = help_label("Number of Bootstrap Samples", "Used in calculating uncertainty but affects run time. 1000 should be used for final versions but fewer may be used in trial versions.")),
          shiny::checkboxInput(inputId = "sen_draft", value = ifelse(meta$set_only, FALSE, TRUE),
                               label = help_label("Mark as Draft", "If checked, a watermark will be included on the output document.")),
          shiny::checkboxInput(inputId = "sen_do_setnets", value = ifelse(meta$set_only, FALSE, TRUE),
                               label = help_label("Make Set Net Harvest Estimate", "Under rare circumstances, you may deem the number of set net interviews insufficient to produce an estimate. In that case, uncheck this box.")),
          shiny::checkboxInput(inputId = "sen_include_plots", value = FALSE,
                               label = help_label("Include Trip Time Plots", "Check if you wish to include the plots showing trip times relative to flight times. Many figures will be displayed but they convey little unique information, so this is optional."))
        ),
