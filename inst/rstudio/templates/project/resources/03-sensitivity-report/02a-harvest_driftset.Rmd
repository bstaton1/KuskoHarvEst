...
harvest_scenarios = lapply(1:nrow(harvest_combos), function(i) {
  ...
  boot_out_set = bootstrap_harvest(interview_data_use, set_effort_info,
                                   gear = "set", n_boot = params$n_boot, stratify_interviews = FALSE)
  
  # combine drift, set, and total into one data frame of bootstrap output
  combine_boot_out(boot_out_drift = boot_out_drift, boot_out_set = boot_out_set)
})

make_harvest_sensitivity_tables(harvest_scenarios, harvest_combos)
```
